...
{
    s[0] = 'f'; //Yes, this takes down the server
...
pthread_t make_thread(int connect_d)
{
    int ret;
    pthread_t thread;

    ret = pthread_create(&thread, NULL, child_code, connect_d);
    if (ret != 0) {
