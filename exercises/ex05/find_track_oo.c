...
Regex *make_regex(char *pattern, int flags) {
    //reformatted version of other code

    //call malloc to make sure we allocated to the heap
    regex_t *regex = malloc(sizeof(regex_t));

    int ret;

    //creates regex object location regex
...
int regex_match(Regex *regex, char *s) {
    ...
    //regexec no return means match, anything else is mismatch/error. Taken from other file
...
void regex_free(Regex *regex) {
    // builtin for freeing regex objects
    regfree(regex);
}
